...
- **2026-03-23** — Added Apple Silicon GRPO template (`scripts/mps_grpo_example.py`): TRL GRPOTrainer + PEFT LoRA on PyTorch MPS — no Unsloth or vLLM required — with 5 reward functions for chain-of-thought math reasoning. Dashboard now reports full memory breakdown on Apple Silicon (MPS `driver_allocated_memory` + `recommended_max_memory`). `demo_server.py` gains `--hardware mps` for Apple Silicon–realistic preview values (1B float16, 18 GB unified memory).
